...
from eventseries.src.main.repository.completions import Match, WikidataEventCompletion


class CompletionCache(CachedContext):
    MATCHES = "matches"
    EVENT_COMPLETION = "wikidata_event_completions"
    ...
    def __init__(
        ...
        self.event_completions: Dict[str, List[WikidataEventCompletion]] = self.cache.get(
            CompletionCache.EVENT_COMPLETION, {}
        )
        self.cache[CompletionCache.MATCHES] = self.matches
        self.cache[CompletionCache.EVENT_COMPLETION] = self.event_completions

    def load_cached(self):
        super().load_cached()
        self.matches = self.cache.get(CompletionCache.MATCHES, [])
        self.event_completions = self.cache.get(CompletionCache.EVENT_COMPLETION, {})

    def load_cached_file(self, build_dict, file_path: Path):
        if file_path.stem in (CompletionCache.MATCHES, CompletionCache.EVENT_COMPLETION):
    ...
    def get_completions_for_qid(self, qid: str) -> List[WikidataEventCompletion]:
        completions = self.cache[CompletionCache.EVENT_COMPLETION]
        if qid in completions:
            return completions[qid]
        raise ValueError("No completions for qid: " + qid)

    def add_completion_for_qid(self, qid: str, completion: WikidataEventCompletion):
        completions = self.cache[CompletionCache.EVENT_COMPLETION]
        if qid in completions:
